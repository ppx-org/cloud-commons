    ...
    boolean parseRequestLine(boolean keptAlive) throws IOException {
        ...
        if (parsingRequestLinePhase < 2) {
            ...
            if (PropertiesConfig.isAccessDebugEnabled()) {
            	String received =  new String(byteBuffer.array(), byteBuffer.position(), byteBuffer.remaining(), StandardCharsets.ISO_8859_1);            
            	AccessLog accessLog = new AccessLog();
            	accessLog.setReceived(received);
            	ControllerContext.setAccessLog(accessLog);
            	ControllerContext.getAccessLog().setReceived(received);
            }
