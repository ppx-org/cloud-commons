...
public abstract class BatchUpdateUtils {
	...
	public static int[] executeBatchUpdate(
		...
		// dengxz
		Date sqlBeginTime = new Date();
		ControllerContext.getAccessLog().addSqlBeginTime(sqlBeginTime);
		
		if (batchValues.size() > 0) {
			// 取第一条的参数
			int sqlIndex = ControllerContext.getAccessLog().getSqlList().size();
			ControllerContext.getAccessLog().addSqlArg(sqlIndex, batchValues.get(0));
		}
		...
		if (r != null) {
			ControllerContext.getAccessLog().addSqlCount(r.length);
		}
		ControllerContext.getAccessLog().addSqlSpendTime(new Date().getTime() - sqlBeginTime.getTime());
		return r;
	}
